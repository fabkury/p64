...
Text: `gfx::fonts` holds the
bundled pixel fonts as glyph tables generated by `tools/gen_fonts.py` from the TTFs in
`assets/fonts` (Capital Hill 6 px, Everyday Slight 5 px, Everyday Standard 6 px,
Everyday Typical 7 px, Everyday Ample 9 px and High Birth 9 px, the regular cut of each,
rendered by Pillow at
their native sizes where they come out crisp; each carries a UI label and whether the
clock overlay offers it; the generated `fonts_data.cpp` is
...
`draw_mask()` draws the same pixels into a mask (the overlay's cached sprite). The
firmware's own text, the status screens (`main/status_screens.cpp`, spec 6.4), uses
`system_font()` (Everyday Standard) and `system_large_font()` (Everyday Ample), stacked
