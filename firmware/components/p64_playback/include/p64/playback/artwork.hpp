...
// fits its canvas into the panel. Produces panel-sized frames one at a time.
#pragma once
...
class Artwork {
 public:
  ...
  bool next_frame(gfx::Frame &out, uint32_t &delay_ms);
  // True once the artwork is known to have a single frame (after its first frame).
  bool is_static() const { return decoder_ && !decoder_->info().animated; }
