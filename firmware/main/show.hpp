...
// to 5.5, 6.1, 15.1). It owns the active playset and its channel indexes, the scheduler,
// the history, the auto-swap timer, pause, and play-this; it hands sources to the
// player and never touches the card itself (the loader does, on its own task). Every
// mutation runs on the main task through a command queue; the API reads snapshots.
#pragma once
