...
- Enclosure: v1 designed and verified against Waveshare's drawings; a development print
  ...
  boards chained on the driver board's SH1.0 GPIO socket). v3 moves the stand's wedge from
  the front edge to a heel under the back, so the front rim is 2 mm on all four sides.
  Neither v2 nor v3 is printed yet; v3 is the current design.
