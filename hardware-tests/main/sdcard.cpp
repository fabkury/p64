...
constexpr size_t kMaxNameChars = 96;
...
bool do_mount() {
...
#if defined(CONFIG_P64_SD_FORMAT_IF_MOUNT_FAILED)
  ...
  if (err != ESP_OK) {
    ...
    if (err == ESP_FAIL) {
      ...
      g_error = "no card answers (is one inserted?)";
    } else {
...
bool mount() {
#if !defined(CONFIG_P64_SD_ENABLE)
...
std::string path_of(const std::string &name) { return std::string(kMountPoint) + "/" + name; }
...
std::vector<FileInfo> list_gifs() {
  ...
  while (dirent *e = readdir(dir)) {
    ...
    if (name.empty() || name.front() == '.' || !has_gif_extension(name)) continue;
    struct stat st = {};
    if (stat(path_of(name).c_str(), &st) != 0 || !S_ISREG(st.st_mode)) continue;
    out.push_back(FileInfo{name, static_cast<size_t>(st.st_size)});
  }
  closedir(dir);
...
bool read_file(const std::string &name, std::vector<uint8_t> &out, size_t max_bytes, std::string &error) {
  ...
  struct stat st = {};
  if (stat(path_of(name).c_str(), &st) != 0 || !S_ISREG(st.st_mode)) {
    error = "no such file on the card";
    return false;
  }
  ...
  FILE *f = std::fopen(path_of(name).c_str(), "rb");
  if (!f) {
...
bool remove_file(const std::string &name, std::string &error) {
  ...
  if (unlink(path_of(name).c_str()) != 0) {
    error = errno == ENOENT ? "no such file on the card" : std::strerror(errno);
    return false;
  }
  return true;
}
