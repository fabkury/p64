...
//
//  v4 (2026-09-11): the USB-C cables no longer plug into the controller
//  from outside. Two JUXINICE panel-mount USB-C extensions (flange
//  25.2 x 8.2 mm with threaded 2.5 mm holes 17 mm apart, housing
//  19.5 mm deep, flat 15 cm ribbon, L-shaped 11 x 11 mm plug) screw to
//  the back face 44 mm either side of centre, 45 mm below it: POWER on
//  the left, USB (programming) on the right. Their plugs sit inside the
//  cavity below the controller's ports, so v2's plug pocket, base groove
//  and back notch are gone. Each housing pokes 2 mm through the frame's
//  back opening. Two printed hooks per ribbon keep it on the cavity back;
//  one groove beside the right socket marks it as USB. Based on v2 (front
//  bar kept, not the v3 plinth).
//
//  v5 (2026-09-11): the speaker shipped with the controller (99.4 x 44.4 x
//  20.7 mm box, two drivers on the front, four corner lugs with 6 mm holes
//  whose back face is 12 mm behind the driver face) sits in the upper half
//  of the back, rear-firing: a plus-shaped window in the back wall lets the
//  8.7 mm of body behind the lug plane into the cavity, the lugs rest on the
//  outer back face and screw from outside into four printed bosses inside
//  (M4 thread-forming), and the drivers face backwards, exposed. The upper
//  vent band is replaced by two slots each side of the speaker; two notches
//  at the window ends pass the lead, which leaves the box in front of the
//  lug plane. Everything else is v4.
// ---------------------------------------------------------------------
...
//  part = "section_usb" -> cut through the left (POWER) socket
//  part = "section_spk" -> cut through the speaker's left lugs and their bosses
...
part = "shell";            // "shell" | "print" | "assembly" | "section_usb" | "section_y" | "section_enc" | "section_spk" | "product"
...
/* [USB sockets] */
usb_sockets = true;
usb_pos     = [[-44, -45], [44, -45]];  // socket centres on the outer back face: [POWER, USB]. 44 mm out puts the housing
                                        // inside the frame's back opening (|x| < 51.9) between the encoder board above
                                        // (bottom edge y = -37.7) and the corner boss below (top y = -51.6)
usb_mark    = [false, true];            // one groove beside the socket = USB (programming); none = POWER
// JUXINICE panel-mount USB-C extension, from the seller's drawings
usb_flange  = [25.2, 8.2];   // flange outline, long axis horizontal
usb_hole_p  = 17;            // threaded 2.5 mm holes; the supplied screws come in from outside
usb_hole_d  = 2.8;           // clearance hole in the wall
usb_mouth   = [9, 3.5];      // USB-C mouth
usb_clr     = 0.3;           // clearance around the mouth
usb_house   = [12, 8.2, 19.5];   // housing behind the flange: width (x), height (y), depth; ribbon exits its far end
usb_plug    = [9, 4.4, 11];      // L-plug body beyond the port face: width, drop below the port, length along the cable
usb_ribbon  = [9, 1.6];          // flat ribbon: width, thickness (estimated from the photos)
usb_keepout = 2.5;               // vent slots closer than this to a flange are dropped
// ribbon guides: two hooks per ribbon on the cavity back, lips facing the ribbon centre line
guides     = true;
ribbon_y   = -50.5;                       // ribbon centre line between plug and socket
guide_pts  = [[[-32, -56.5], [-22, -44.5]], [[32, -56.5], [22, -44.5]]];   // per socket, [x, y] on the cavity back
guide_w    = 3;      // hook width along x
guide_t    = 1.6;    // post thickness
guide_h    = 4;      // total height from the cavity back
guide_lt   = 1.5;    // lip thickness; gap under the lip = guide_h - guide_lt
guide_lip  = 2.5;    // lip reach over the ribbon

/* [Speaker] */
speaker    = true;
spk_pos    = [0, 27];          // centre of the speaker envelope on the outer back face; 27 keeps the window 2.6 mm
                               // below the top screw bosses and the sunk body 4.6 mm behind the panel's VH4 socket
spk_env    = [99.4, 44.4];     // envelope over the lugs (x, y), measured 2026-09-11 (input/measurements.md)
spk_gap    = [82.6, 27.5];     // clear span between the lug pairs: between the end lugs (x), between the side lugs (y)
spk_depth  = 20.7;             // driver face to back
spk_lug_z  = 12;               // driver face to the lug's back face, the face that rests on the shell
spk_lug_t  = 1.8;
spk_hole_d = 6.0;              // lug holes, assumed centred in the lugs
spk_clr    = 0.5;              // window clearance per side
spk_screw  = 3.6;              // pilot for M4 thread-forming screws (M4 x 10)
spk_boss_d = 8;                // 2.2 mm walls around the pilot; clipped where it would meet the window
spk_boss_h = 7;                // below the inner wall face
spk_cable  = [4.5, 6];         // lead notch at each window end: width (y), reach beyond the window edge (x)
spk_vents  = [[57.5, 27, 34], [61.5, 27, 34], [-57.5, 27, 34], [-61.5, 27, 34]];   // [x, centre y, length]: upper exhaust beside the box
...
vent_pitch      = 6;
vent_xmax       = 42;
vent_bands      = [[-28, 36]];             // [centre y, slot length]; lower band only, the speaker takes the upper half
...
// speaker: lug size, hole centres (relative to spk_pos, in the back-face plane), sunk depth
spk_lug   = [(spk_env[0] - spk_gap[0]) / 2, (spk_env[1] - spk_gap[1]) / 2];
spk_holes = [for (sx = [-1, 1], sy = [-1, 1]) [sx * (spk_env[0] - spk_lug[0]) / 2, sy * (spk_env[1] - spk_lug[1]) / 2]];
spk_rear  = spk_depth - spk_lug_z;           // body behind the lug plane, inside the shell
...
// a vent slot (column x, band centre y0, length l) is dropped when it would run into an encoder board or a socket flange
...
function enc_blocks(x, y0, l) =
    ...
    (usb_sockets && len([for (p = usb_pos) if (rect_blocks(x, y0, l, p, usb_flange[0], usb_flange[1], usb_keepout)) 1]) > 0);
...
if (speaker) {
    echo(str("speaker window y ", spk_pos[1] - spk_env[1]/2 - spk_clr, "..", spk_pos[1] + spk_env[1]/2 + spk_clr,
             " (top bosses start at y ", 56.85 - boss_od/2, ")  lugs ", spk_lug, "  hole centres ", [for (h = spk_holes) h + spk_pos],
             "  rear face z at the window bottom ", z_back(spk_pos[1] - spk_env[1]/2) - (back_t + spk_rear) * cos(back_ang),
             " (VH4 socket top +1, HUB75 OUT header top -3.1)  proud of the back ", spk_lug_z));
}
if (usb_sockets) for (i = [0 : 1]) { p = usb_pos[i];
    echo(str("socket ", i == 0 ? "POWER" : "USB", " at ", p, ": housing x ", p[0] - usb_house[0]/2, "..", p[0] + usb_house[0]/2,
             " (frame opening +-51.9), front end z ", z_cav(p[1]) - usb_house[2] * cos(back_ang), " (frame back 0, panel PCB -12)",
             "  flange to encoder board ", (enc_pos[i][1] - enc_board/2) - (p[1] + usb_flange[1]/2),
             "  flange to corner boss ", (p[1] - usb_flange[1]/2) - (-56.85 + boss_od/2)));
    echo(str("plug ", i == 0 ? "POWER" : "USB", " at port ", usb[i], ": body y ", usb[i][1] - usb_plug[1], "..", usb[i][1],
             " (bottom wall ", -half_in, ")  ribbon exit towards the back, cavity back z there ", z_cav(usb[i][1] - usb_plug[1])));
}
...
// ---------------- speaker ----------------
module spk_window_2d() {                // plus shape: the body passes, the four corner lugs rest on the wall
    rrect(spk_env[0] + 2*spk_clr, spk_gap[1] + 2*spk_clr, 1);
    rrect(spk_gap[0] + 2*spk_clr, spk_env[1] + 2*spk_clr, 1);
}

module spk_window() {
    on_back(spk_pos[1]) translate([spk_pos[0], 0, -back_tz - 1]) {
        linear_extrude(back_tz + 2) spk_window_2d();
        for (s = [-1, 1])              // lead notch, centred on each end edge
            translate([s * (spk_env[0]/2 + spk_clr + spk_cable[1]/2 - 0.5), 0, 0])
                linear_extrude(back_tz + 2) square([spk_cable[1] + 1, spk_cable[0]], center = true);
    }
}

module spk_bosses() {                   // hang from the inner wall face behind each lug, clipped clear of the window
    on_back(spk_pos[1]) translate([spk_pos[0], 0, -back_t + 0.01]) mirror([0, 0, 1]) difference() {
        for (h = spk_holes) translate([h[0], h[1], 0]) cylinder(d = spk_boss_d, h = spk_boss_h);
        translate([0, 0, -1]) linear_extrude(spk_boss_h + 2) spk_window_2d();
    }
}

module spk_holes_cut() {                // M4 pilot through wall and boss
    on_back(spk_pos[1]) translate([spk_pos[0], 0, 0]) for (h = spk_holes)
        translate([h[0], h[1], -(back_t + spk_boss_h + 1)]) cylinder(d = spk_screw, h = back_t + spk_boss_h + 2);
}

// ---------------- USB sockets and ribbon guides ----------------
module usb_cutout(p) {                  // stadium-shaped mouth plus the two screw clearance holes, through the back wall
    on_back(p[1]) translate([p[0], 0, -back_tz - 1]) {
        linear_extrude(back_tz + 2) rrect(usb_mouth[0] + 2*usb_clr, usb_mouth[1] + 2*usb_clr, usb_mouth[1]/2 + usb_clr - 0.01);
        for (s = [-1, 1]) translate([s*usb_hole_p/2, 0, 0]) cylinder(d = usb_hole_d, h = back_tz + 2);
    }
}

module usb_groove(p) {                  // one groove outboard of the socket: marks the USB (programming) socket
    sx = p[0] > 0 ? 1 : -1;
    on_back(p[1]) translate([p[0] + sx*(usb_flange[0]/2 + mark_gap + mark_w/2), 0, -mark_d])
        linear_extrude(mark_d + 1) square([mark_w, mark_l], center = true);
}

module hook(p) {                        // post on the cavity back with a lip over the ribbon's edge, lip towards ribbon_y
    dir = ribbon_y > p[1] ? 1 : -1;
    on_back(p[1]) translate([p[0], 0, -back_t + 0.01]) mirror([0, 0, 1]) {   // +z now points into the cavity
        translate([-guide_w/2, dir > 0 ? 0 : -guide_t, 0]) cube([guide_w, guide_t, guide_h]);
        translate([-guide_w/2, dir > 0 ? 0 : -(guide_t + guide_lip), guide_h - guide_lt]) cube([guide_w, guide_t + guide_lip, guide_lt]);
    }
}
...
module vent_grid() {
    ...
    if (speaker) for (v = spk_vents) vent_slot(v[0], v[1], v[2]);
}
...
module shell() {
    difference() {
        union() {
            ...
            if (usb_sockets && guides) for (g = guide_pts, p = g) hook(p);
            if (speaker) spk_bosses();
        }
        for (p = holes) boss_hole(p);
        if (speaker) { spk_window(); spk_holes_cut(); }
        if (usb_sockets) for (i = [0 : 1]) { usb_cutout(usb_pos[i]); if (usb_mark[i]) usb_groove(usb_pos[i]); }
        if (vents) vent_grid();
...
module ghost_usb() {   // JUXINICE sockets on the back face, L-plugs in both ports, the guided ribbon sections
    zc = z_chip + chip_t + 1.6;                      // port axis height
    for (i = [0 : 1]) {
        p = usb_pos[i]; q = usb[i];
        on_back(p[1]) translate([p[0], 0, 0]) {
            color("black", 0.8) linear_extrude(2) rrect(usb_flange[0], usb_flange[1], usb_flange[1]/2 - 0.01);   // flange
            color("black", 0.6) translate([-usb_house[0]/2, -usb_house[1]/2, -back_t - usb_house[2]]) cube([usb_house[0], usb_house[1], usb_house[2]]);
        }
        color("black", 0.6) translate([q[0] - usb_plug[0]/2, q[1] - usb_plug[1], zc - 3.5]) cube([usb_plug[0], usb_plug[1], usb_plug[2]]);   // plug body
        g = guide_pts[i];
        color("silver", 0.6) on_back(ribbon_y) translate([min(g[0][0], g[1][0]) - 6, -usb_ribbon[0]/2, -back_t - usb_ribbon[1]])   // guided ribbon section
            cube([abs(g[0][0] - g[1][0]) + 12, usb_ribbon[0], usb_ribbon[1]]);
    }
}
...
module ghost_speaker() {   // the box: lug plane on the outer back face, 8.7 mm sunk, 12 mm proud, drivers facing back
    on_back(spk_pos[1]) translate([spk_pos[0], 0, 0]) {
        color([0.12, 0.12, 0.12]) translate([0, 0, -spk_rear]) linear_extrude(spk_rear + 0.01) offset(r = -0.6) spk_window_2d();   // sunk part
        color([0.12, 0.12, 0.12]) linear_extrude(spk_lug_z) rrect(spk_env[0], spk_env[1], 2);                                    // outer part with the lugs
        color([0.25, 0.25, 0.25]) for (s = [-1, 1]) translate([s * 24, 0, spk_lug_z]) cylinder(d = 36, h = 0.6);                 // driver surrounds
        color("black") translate([24, 0, spk_lug_z + 0.6]) cylinder(d = 26, h = 0.4);                                            // passive radiator
        color("black") translate([-24, 0, spk_lug_z + 0.6]) sphere(d = 18);                                                     // dome
        color("silver") for (h = spk_holes) translate([h[0], h[1], spk_lug_z - spk_lug_t]) cylinder(d = 7, h = spk_lug_t + 0.5);  // screw heads
    }
}

module ghosts() { ghost_panel(); ghost_chip(); if (usb_sockets) ghost_usb(); if (speaker) ghost_speaker(); if (encoders) for (i = [0 : len(enc_pos) - 1]) ghost_encoder(enc_pos[i], enc_rot[i]); }
...
if (part == "section_usb") intersection() { union() { shell(); ghosts(); }
                                            translate([usb_pos[0][0] - 200, -200, -100]) cube([200, 400, 200]); }
if (part == "section_spk") intersection() { union() { shell(); ghosts(); }
                                            translate([spk_pos[0] + spk_holes[0][0] - 200, -200, -100]) cube([200, 400, 200]); }
