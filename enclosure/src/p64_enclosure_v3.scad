...
//
//  v3 (2026-09-11): the wedge foot moves from the front-bottom edge to
//  the back-bottom edge (foot = "back"). The front then shows the same
//  2 mm rim on all four sides instead of a 9 mm bar under the LEDs; the
//  extra material sits as a heel under the rear half, hidden from the
//  front. The base is still one flat plane at 12 deg spanning the full
//  depth, so the stance is unchanged. foot = "front" gives the v2 shape.
// ---------------------------------------------------------------------
...
foot         = "back"; // where the wedge that makes the base flat sits: "back" = heel under the rear half (v3),
                       // "front" = bar below the front edge (v1/v2)
...
wedge_f  = foot == "front" ? (depth_bottom + lip) * tan(tilt) : 0;   // wedge below the front outline (v1/v2)
wedge_b  = foot == "back"  ? (depth_bottom + lip) * tan(tilt) / (1 - tan(tilt) * tan(back_ang)) : 0;
                                                               // heel below the back-bottom edge; its back lies in the
                                                               // sloping back plane, which adds a little depth, hence the divisor
wedge    = max(wedge_f, wedge_b);                              // how far the shell reaches below the pocket outline
...
echo(str("outer size X x Y: front outline ", 2*half_out, " x ", 2*half_out + wedge_f, ", back face ", 2*half_out, " x ", 2*half_out + wedge_b,
         "  depth bottom/top = ", depth_bottom + lip, "/", depth_top + lip, "  back slope = ", back_ang, " deg",
         "  foot = ", foot, " (front wedge ", wedge_f, ", rear heel ", wedge_b, ")"));
...
module back_slab(th = 0.01) {          // thin slab whose top face is the inclined back plane
    H  = 2*half_out / cos(back_ang);   // back face height measured along its slope
    Hb = wedge_b / cos(back_ang);      // heel: how far the back face continues below the bottom edge
    translate([0, 0, z_mid]) rotate([-back_ang, 0, 0]) translate([0, -Hb/2, -th])
        linear_extrude(th) rrect(2*half_out, H + Hb, r_out);
}
...
module standing() {
    translate([0, 0, (half_out + wedge_f) * cos(tilt) - lip * sin(tilt)])
