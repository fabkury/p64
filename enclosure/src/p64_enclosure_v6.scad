...
ad_clr      = [0.3, 0.5];   // cradle clearance per side in x and y (y is looser: the boot gap below is estimated)
ad_win_clr  = [0.4, 0.6];   // window clearance per side in x and y; 0.1 more than the cradle so there is no step
ad_cradle_d = 6;       // cradle depth in front of the cavity back
ad_cradle_t = 2;       // cradle wall thickness
ad_chamfer  = 1;       // 45-degree chamfer on the window's outer edge and on the cradle's entry
...
ad_in     = [ad_pair_w + 2*ad_clr[0], ad_t + 2*ad_clr[1], ad_r + ad_clr[1]];       // cradle opening: w, h, corner r
ad_win    = [ad_pair_w + 2*ad_win_clr[0], ad_t + 2*ad_win_clr[1], ad_r + ad_win_clr[1]];   // wall window: w, h, corner r
ad_y_top  = ad_ctr[1] + ad_in[1]/2;                                     // top of the cradle's side walls
ad_win_y  = ad_ctr[1] + back_t * sin(back_ang);                         // on_back origin that centres the window on the pair at the INNER face
...
// gap on the outer face between the window's chamfer outline (corner arcs of radius ad_win[2] + ad_chamfer) and the mic1 hole
ad_corner_c  = [ad_ctr[0] + ad_win[0]/2 - ad_win[2], ad_win_y + ad_win[1]/2 - ad_win[2]];
ad_mic_gap   = norm(mic1 - ad_corner_c) - (ad_win[2] + ad_chamfer) - mic_d/2;
...
if (adapters) {
    ...
    echo(str("adapters: window ", ad_win[0], " x ", ad_win[1], " (+", ad_chamfer, " chamfer)  cradle x ", ad_ctr[0] - ad_in[0]/2 - ad_cradle_t,
             "..", ad_ctr[0] + ad_in[0]/2 + ad_cradle_t, "  cradle z ", ad_zc - ad_cradle_d, "..", ad_zc, "  side walls top y ", ad_y_top,
             "  mic1 hole edge above the cradle ", (mic1[1] - mic_d/2) - ad_y_top, "  window chamfer to mic1 hole ", ad_mic_gap));
    echo(str("adapters: female faces to the cavity back at the bodies' top edge ", z_cav(ad_ctr[1] + ad_t/2) - ad_z1,
             "  at the bottom edge ", z_cav(ad_y_bot) - ad_z1, " (the window is cut there, so only the cradle walls can touch)"));
    echo(str("frame notch, panel native orientation (arrows up, seen from the back): x ", frame_wall_y, "..", -frame_open,
             "  y ", -(usb[1][0] + ad_w/2 + 1), "..", -(usb[0][0] - ad_w/2 - 1), "  depth >= ", -ad_z0 + 0.5, " (through the plate)"));
}
...
module ad_open2d() { rrect(ad_in[0], ad_in[1], ad_in[2]); translate([-ad_in[0]/2, 0]) square([ad_in[0], 30]); }   // opening, open towards the controller
...
module ad_window() {                    // through the back wall over both socket faces, chamfered outside
    on_back(ad_win_y) translate([ad_ctr[0], 0, 0]) {
        ...
        hull() {
            translate([0, 0, -ad_chamfer]) linear_extrude(0.01) ad_win2d();
            translate([0, 0, 1])           linear_extrude(0.01) offset(r = ad_chamfer + 1) ad_win2d();
        }
    }
}

module ad_cradle() {                    // U on the cavity back around the last ad_cradle_d mm of the bodies: bottom + two sides,
    z0   = ad_zc - ad_cradle_d;         // open towards the controller's edge; the bottom wall merges into the shell's bottom wall
    ztop = z_back(ad_y_top) - 0.2;      // ends inside the back wall (solid there anyway; the window cuts through it)
    yb   = -half_in - 0.5;              // 0.5 mm into the bottom wall
    ow   = ad_in[0] + 2*ad_cradle_t;
    intersection() {
        difference() {
            translate([ad_ctr[0] - ow/2, yb, z0]) cube([ow, ad_y_top - yb, ztop - z0]);
            translate([ad_ctr[0], ad_ctr[1], z0 - 1]) linear_extrude(ztop - z0 + 2) ad_open2d();
            hull() {                    // entry chamfer for the bodies sliding in with the panel
                translate([ad_ctr[0], ad_ctr[1], z0 - 0.01])       linear_extrude(0.01) offset(r = ad_chamfer) ad_open2d();
                translate([ad_ctr[0], ad_ctr[1], z0 + ad_chamfer]) linear_extrude(0.01) ad_open2d();
            }
        }
        outer_body();
    }
...
module ad_groove() {                    // one groove beside the right end of the window: marks the USB (programming) socket
    on_back(ad_win_y) translate([ad_ctr[0] + ad_win[0]/2 + ad_chamfer + mark_gap + mark_w/2, 0, -mark_d])
...
module notch_block() {   // the material to remove from the frame's back plate: the strip between opening and wall, 1 mm wider than the pair
    translate([ad_ctr[0] - ad_pair_w/2 - 1, frame_wall_y - 0.01, -frame_plate_t - 0.01]) cube([ad_pair_w + 2, -frame_wall_y - frame_open + 0.02, frame_plate_t + 1]);
}
...
module ghosts() { ghost_panel(); ghost_chip(); if (adapters) { ghost_adapters(); ghost_cable(); }
